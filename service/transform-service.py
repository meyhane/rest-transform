...
authorization = json.loads(os.environ.get("AUTHORIZATION", "{}"))
...
if authorization:
    if authorization.get("type", "") == "oauth2":
        session_factory = Oauth2System({"oauth2": authorization["oauth2"], "headers": headers})
    else:
    ...
    def generate(entities):
        ...
        for index, entity in enumerate(entities):
            ...
            with session_factory.make_session() as s:
                logger.debug(f"url={url}, headers={headers}, entity={entity}")
                if method == "GET":
                    resp = s.get(url, headers=headers)
                else:
                     resp = s.request(method, url, data=entity.get("payload"),
                                                headers=headers)
                logger.debug(f'transform of entity _id={entity.get("_id","?")} received {resp.status_code}')
